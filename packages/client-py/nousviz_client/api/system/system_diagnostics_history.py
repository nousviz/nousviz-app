from http import HTTPStatus
from typing import Any

import httpx

from ... import errors
from ...client import AuthenticatedClient, Client
from ...models.diagnostics_history_response import DiagnosticsHistoryResponse
from ...models.error_detail import ErrorDetail
from ...models.http_validation_error import HTTPValidationError
from ...models.rbac_error_detail import RBACErrorDetail
from ...types import UNSET, Response, Unset


def _get_kwargs(
    *,
    id: str,
    days: int | Unset = 30,
) -> dict[str, Any]:

    params: dict[str, Any] = {}

    params["id"] = id

    params["days"] = days

    params = {k: v for k, v in params.items() if v is not UNSET and v is not None}

    _kwargs: dict[str, Any] = {
        "method": "get",
        "url": "/api/system/diagnostics/history",
        "params": params,
    }

    return _kwargs


def _parse_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
) -> DiagnosticsHistoryResponse | ErrorDetail | HTTPValidationError | RBACErrorDetail | None:
    if response.status_code == 200:
        response_200 = DiagnosticsHistoryResponse.from_dict(response.json())

        return response_200

    if response.status_code == 400:
        response_400 = ErrorDetail.from_dict(response.json())

        return response_400

    if response.status_code == 401:
        response_401 = ErrorDetail.from_dict(response.json())

        return response_401

    if response.status_code == 403:
        response_403 = RBACErrorDetail.from_dict(response.json())

        return response_403

    if response.status_code == 422:
        response_422 = HTTPValidationError.from_dict(response.json())

        return response_422

    if client.raise_on_unexpected_status:
        raise errors.UnexpectedStatus(response.status_code, response.content)
    else:
        return None


def _build_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
) -> Response[DiagnosticsHistoryResponse | ErrorDetail | HTTPValidationError | RBACErrorDetail]:
    return Response(
        status_code=HTTPStatus(response.status_code),
        content=response.content,
        headers=response.headers,
        parsed=_parse_response(client=client, response=response),
    )


def sync_detailed(
    *,
    client: AuthenticatedClient | Client,
    id: str,
    days: int | Unset = 30,
) -> Response[DiagnosticsHistoryResponse | ErrorDetail | HTTPValidationError | RBACErrorDetail]:
    """Presence-per-snapshot history for one diagnostic finding (B273)

     Return [{snapshot_at, present, severity}] over the last N days
    for one diagnostic finding. The frontend renders this as a
    timeline strip on the expanded FindingCard.

    Snapshots without the finding return `present=false, severity=null`.

    Args:
        id (str):
        days (int | Unset):  Default: 30.

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Response[DiagnosticsHistoryResponse | ErrorDetail | HTTPValidationError | RBACErrorDetail]
    """

    kwargs = _get_kwargs(
        id=id,
        days=days,
    )

    response = client.get_httpx_client().request(
        **kwargs,
    )

    return _build_response(client=client, response=response)


def sync(
    *,
    client: AuthenticatedClient | Client,
    id: str,
    days: int | Unset = 30,
) -> DiagnosticsHistoryResponse | ErrorDetail | HTTPValidationError | RBACErrorDetail | None:
    """Presence-per-snapshot history for one diagnostic finding (B273)

     Return [{snapshot_at, present, severity}] over the last N days
    for one diagnostic finding. The frontend renders this as a
    timeline strip on the expanded FindingCard.

    Snapshots without the finding return `present=false, severity=null`.

    Args:
        id (str):
        days (int | Unset):  Default: 30.

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        DiagnosticsHistoryResponse | ErrorDetail | HTTPValidationError | RBACErrorDetail
    """

    return sync_detailed(
        client=client,
        id=id,
        days=days,
    ).parsed


async def asyncio_detailed(
    *,
    client: AuthenticatedClient | Client,
    id: str,
    days: int | Unset = 30,
) -> Response[DiagnosticsHistoryResponse | ErrorDetail | HTTPValidationError | RBACErrorDetail]:
    """Presence-per-snapshot history for one diagnostic finding (B273)

     Return [{snapshot_at, present, severity}] over the last N days
    for one diagnostic finding. The frontend renders this as a
    timeline strip on the expanded FindingCard.

    Snapshots without the finding return `present=false, severity=null`.

    Args:
        id (str):
        days (int | Unset):  Default: 30.

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Response[DiagnosticsHistoryResponse | ErrorDetail | HTTPValidationError | RBACErrorDetail]
    """

    kwargs = _get_kwargs(
        id=id,
        days=days,
    )

    response = await client.get_async_httpx_client().request(**kwargs)

    return _build_response(client=client, response=response)


async def asyncio(
    *,
    client: AuthenticatedClient | Client,
    id: str,
    days: int | Unset = 30,
) -> DiagnosticsHistoryResponse | ErrorDetail | HTTPValidationError | RBACErrorDetail | None:
    """Presence-per-snapshot history for one diagnostic finding (B273)

     Return [{snapshot_at, present, severity}] over the last N days
    for one diagnostic finding. The frontend renders this as a
    timeline strip on the expanded FindingCard.

    Snapshots without the finding return `present=false, severity=null`.

    Args:
        id (str):
        days (int | Unset):  Default: 30.

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        DiagnosticsHistoryResponse | ErrorDetail | HTTPValidationError | RBACErrorDetail
    """

    return (
        await asyncio_detailed(
            client=client,
            id=id,
            days=days,
        )
    ).parsed
